...
PACKAGE = Path(__file__).parent.parent / "prepare_codestripper"
...
def _imported_modules() -> Set[str]:
    ...
    return {module for module in modules
            if module not in sys.stdlib_module_names and module != "prepare_codestripper"}


def _declared_dependencies() -> Set[str]:
    declared: Set[str] = set()
    for line in requires("prepare-codestripper") or []:
